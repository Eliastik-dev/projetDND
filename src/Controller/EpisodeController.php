...
final class EpisodeController extends AbstractController
{
    ...
    #[Route('/choose-action/{id}', name: 'app_episode_choose_action', methods: ['POST'])]
    public function chooseAction(int $id): Response
    {
        $choice = $this->entityManager->getRepository(Choice::class)->find($id);
        if (!$choice) {
            throw $this->createNotFoundException('Choice not found');
        }

        $race = $this->entityManager->getRepository(Race::class)->find($choice->getRaceId());
        if (!$race) {
